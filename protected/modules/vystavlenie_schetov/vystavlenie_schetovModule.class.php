...
final class vystavlenie_schetovModule extends \Fastest\Core\Modules\Module
{
    ...
    public function blockMethod()
    {
		...
		if (!empty($_POST['renter']) && !empty($_POST['date']) && !empty($_POST['year']) && !empty($_POST['month'])){
			...
			foreach ($_POST['renters'] as $key => $value) {				
                ...
                $schet_last_id = mysql_insert_id(); // Получаем id последнего добавленного счета
				...
				exit(__($_POST));
				$akt_last_id = mysql_insert_id(); // Получаем id последнего добавленного акта
